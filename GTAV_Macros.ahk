...
SnackMenuKey         := "+^" ; 零食菜单 (+ = shift, rtfm).
AutoHealthKey        := "+!" ; 自动零食，从第二个零食槽吃两个零食
ArmorMenuKey         := "+@" ; 打开装甲菜单
AutoArmorKey         := "+#" ; 自动装甲装备（仅使用超重型装甲）
RetrieveCarKey       := "+$" ; 请求当前激活的个人车辆
ToggleRadarKey       := "+F2" ; 在扩展雷达和标准雷达之间切换
CEOBuzzardKey        := "F3" ; ceo的秃鹫
RequestSparrowKey    := "+%" ; 召唤你的麻雀（或者你最后一次要求的月亮池交通工具）
ReturnSparrowKey     := "^%" ; 把你的麻雀送回
ForceDisconnectKey   := "F12" ; 通过暂停过程10秒强制断开，需要PSSUPPEND.exe
KillGameKey          := "+F12" ; Kill game process, requires pskill.exe
ToggleVIPKey         := "NumpadMult" ; 切换VIP模式（VIP/CEO/MC时需要）。如果使用ManualInventoryLocation选项，则无效。
ToggleCPHKey         := "^NumpadMult" ; 切换Cayo Perico盗窃最终模式（额外菜单项），另请参见DoToggleCPHWithVIP。如果使用ManualInventoryLocation选项，则无效。
ToggleAFKKey         := "+NumpadMult" ; 切换AFK模式
TogglePassiveKey     := "F24" ; 切换被动模式.
ToggleClickerKey     := "F24" ; 切换点击器（XButton2=Mouse5）
ToggleAutoHeliKey    := "F24" ; 保持油门和俯仰向前按下，首先将直升机带到足够的高度，然后将其用作自动驾驶仪
ChatSnippetsKey      := "F24" ; 给你一些文本片段放进聊天室（聊天室必须已经打开）
CycleOutfitKey       := "F24" ; 装备下一个/循环使用保存的装备
RandomHeistKey       := "F24" ; 从手机选项中选择随叫随到的随机抢劫
EquipScarfKey        := "F24" ; Equip first scarf (heist outfit glitch, see readme/misc)

DialDialogKey        := "+F5" ; 用几乎所有数字的列表调用GUI
CallMechanicKey      := "F5" ; 打电话给技工
CallPegasusKey       := "F24" ; 打电话给帕伽索斯
CallMerryweatherKey  := "F24" ; 打电话给梅林军火商
CallInsuranceKey     := "F6" ; 共荣保险
CallLesterKey        := "^1" ; ctrl+1打电话给莱斯特
CallAssistantKey     := "^F6" ; 呼叫助理
